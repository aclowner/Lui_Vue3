...
<div class="lui-select">
    ...
    <div class="_scroll sel-list" v-show="show" tabindex="99" @blur="show=false" @mousedown="evTag='lmd'">
        <ul class='_con'>
            ...
            <template v-else>                    
                <li v-for="(lo,li) in optionArrShow" :key="li" :class="{active:lo[0]==modelValue}" @click="liClick(lo)">{{lo[1]}}</li>
            </template>
        </ul>
    </div>
</div>
