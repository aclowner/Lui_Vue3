...
<div :class="['lui-layer','cover'+cover,{'layer-alert':alertOpen}]" v-show="ppShow.length>0||alertOpen">
    <transition-group mode="in-out" name="popuping" >
        <component v-for="(pp,pi) in ppShow" :key="pp+pi" :ref="pp" :is="transName(pp)" @close="close($event,pi)" @close-all="ppShow = []" @open="open" @active="ppActive=pi" @sibling="sibling" @send="send" @alert="alert" @confirm="confirm" :class="[{'show-active':ppActive==pi}]"></component>
        <lui-popup-alert v-if="alertOpen" key="lui-alert" :btns="alertBtn" :con="alertCon" @alert="alertBtnEvent"></lui-popup-alert>
...
import LuiPopupAlert from "./LuiPopupAlert"

import popups from "../../views/popup"
popups.LuiPopupAlert = LuiPopupAlert;
    ...
    methods:{
        ...
        child(pp,param){  //调用子组件函数，data
            ...
            for(let key in param){   
                ...
                if(key==="_close_")
                    this.$refs[pp].$emit("close");
                //pp目标组件名
                //key目标组件需要被操作的对象
                let cv = this.$refs[pp][key];                                 
                if(typeof(cv)=="function")                          //函数-调用
                    ...
                    Object.assign(this.$refs[pp][key],param[key]);
                else //非对象变量
                    this.$refs[pp][key] = param[key];            //值类型-赋值  
            }
        },
        childPrev(param){
            //当前激活弹出层的前一个弹出层
